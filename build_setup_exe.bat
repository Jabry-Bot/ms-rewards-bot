...
if not defined BUILD_PY (
    ...
    echo         Crea el venv del bot ^(setup.bat^) o instala Python en el PATH.
    pause
    exit /b 1
)
...
"%BUILD_PY%" -m PyInstaller --noconfirm --clean --onefile --windowed --uac-admin --name setup --collect-all customtkinter --paths "%ROOT%" run_setup.py
if errorlevel 1 (
...
if exist "%ROOT%dist\setup.exe" (
    ...
    echo asi que debe vivir junto a ms_rewards\.
) else (
